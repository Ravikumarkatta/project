...
from src.serve.verse_resolver import VerseReference, VerseResolver
...
def test_validate_reference(verse_resolver):
    ...
    assert not verse_resolver.validate_reference(
        VerseReference("Genesis", 100, 1)
    )  # Invalid chapter
    assert not verse_resolver.validate_reference(
        VerseReference("Genesis", 1, 100)
    )  # Invalid verse
    assert not verse_resolver.validate_reference(
        VerseReference("InvalidBook", 1, 1)
    )  # Invalid book
