...
class BibleProcessor:
    ...
    def normalize_book_name(self, name: str) -> Optional[str]:
        ...
        if lower_name in ["samuel", "kings", "chronicles", "corinthians", 
    ...
    def standardize_book_titles(self, text: str) -> str:
        ...
        for line in lines:
            ...
            if book_name:
                ...
                if book_name in BIBLE_BOOKS and (self.include_apocrypha or 
                    ...
                    logging.debug(f"Standardized book title: {original_line} -> {book_name}")
                else:
        ...
        logging.info(f"Standardized {processed_line_count} book titles and chapter headers")
        return "\n".join(result_lines)
    ...
    def validate_bible_structure(self) -> Dict[str, Any]:
        ...
        return report
    def process_directory(self, input_dir: str, output_dir: str, output_format: str = "text",
        ...
        for txt_file in txt_files:
           ...
           output_path = os.path.join(output_dir, 
                                   f"{os.path.splitext(txt_file)[0]}.{output_format}")
        
           logging.info(f"Processing file: {input_path}")
           report = self.process_bible_text(input_path, output_path, output_format, validate)
        ...
        try:
           ...
           with open(input_file, 'r', encoding='utf-8') as f:
            while True:
                ...
                logging.debug(f"Processed {len(processed_text)/file_size*100:.1f}% of file")
    ...
    def write_output(self, text: str, output_file: str, output_format: str = "text") -> None:
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Process KJV Bible text into standardized format")
    parser.add_argument("--input", required=True, help="Path to the input KJV Bible text file")
    ...
    parser.add_argument("--validation-report", help="Path to save detailed validation report as JSON")
    ...
    if args.batch:
        ...
        reports = processor.process_directory(args.input, args.output, args.format, args.validate)
        ...
        if args.validate and report['validation']:
            print("\nValidation Report:")
            print(f"- Valid structure: {'Yes' if report['validation']['valid'] else 'No'}")
            ...
            if report['validation']['warnings']:
                ...
                if len(report['validation']['warnings']) > 3:
                    print(f"  * ... and {len(report['validation']['warnings']) - 3} more warnings")
            else:
                print("- No warnings")
