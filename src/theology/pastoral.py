...
class PastoralSensitivity:
        ...
        try:
            ...
            if not rules_file.is_file(): # Check if it's a file specifically
                ...
                if rules_path == "config/theological_rules.json":
                     self.logger.error(f"Default rules file not found or not a file: {rules_path}. Pastoral checks may be limited.")
            ...
            self.logger.exception(f"Invalid JSON in {rules_path}. Cannot load pastoral rules.")
            ...
            self.logger.exception(f"Failed to load or parse rules from {rules_path}: {e}")
    ...
    def analyze_sensitivity(
        ...
        if not isinstance(text, str) or not text.strip():
            self.logger.warning("analyze_sensitivity called with empty or invalid text.")
            # Return a default structure indicating failure/no analysis
            return {"sensitive": False, "topics": [], "suggestions": ["Input text was empty."], "score": 0.0}
        ...
        for topic_id, topic_rules in self.sensitivity_topics.items():
            ...
            if isinstance(topic_rules, dict):
                topic_result = self._analyze_sensitive_topic(text_lower, topic_id, topic_rules)
                if topic_result["detected"]:
                 ...
                 self.logger.warning(f"Invalid rule structure for topic '{topic_id}'. Expected dict, got {type(topic_rules)}. Skipping.")
    ...
    def _analyze_sensitive_topic(
        ...
        if isinstance(keywords, list):
            for keyword in keywords:
             ...
             self.logger.warning(f"Keywords for topic '{topic_id}' is not a list. Skipping keyword check.")
        ...
        if isinstance(encouraged_phrases, list):
             encouraged_count = sum(
                 1 for phrase in encouraged_phrases
                 if isinstance(phrase, str) and phrase and re.search(rf"\b{re.escape(phrase.lower())}\b", text_lower)
             )
             total_encouraged = len([p for p in encouraged_phrases if isinstance(p, str) and p]) # Count valid phrases
             ...
             self.logger.warning(f"Encouraged phrases for topic '{topic_id}' is not a list. Skipping check.")
            ...
            for phrase in avoid_phrases:
                if isinstance(phrase, str) and phrase:
                    ...
                    if re.search(rf"\b{re.escape(phrase.lower())}\b", text_lower):
                        ...
                        context_snippet = self._extract_phrase_context(text_lower, phrase.lower())
                        suggestions.append(f"Avoid using '{phrase}' when discussing {topic_id}. Found in context: {context_snippet}")
        else:
             self.logger.warning(f"Avoid phrases for topic '{topic_id}' is not a list. Skipping check.")
    ...
    def _check_life_situation(self, text_lower: str, context: str) -> Dict[str, Any]:
        ...
        if not isinstance(situation, dict) or not situation:
            self.logger.debug(f"No rules found or invalid structure for life situation context: '{context}'")
        ...
        if isinstance(required, list):
            for element_list in required:
                ...
                if isinstance(element_list, list) and all(isinstance(req, str) for req in element_list):
                    # Robustness: Use regex word boundaries
                    found_alternative = any(
                        re.search(rf"\b{re.escape(req.lower())}\b", text_lower)
                        for req in element_list if req # Check req is not empty
                    )
                    if not found_alternative:
                        score += self._SITUATION_REQUIRED_MISSING_PENALTY # Use constant
                        # BUG FIX: Check if element_list is non-empty before accessing [0]
                        if element_list:
                            suggestions.append(f"Include language related to '{element_list[0]}' when addressing {context}")
                        else:
                            suggestions.append(f"Missing required element when addressing {context} (check rule config)")
                else:
                     self.logger.warning(f"Invalid 'required_elements' item for context '{context}'. Expected list of strings, got: {element_list}")
        else:
             self.logger.warning(f"'required_elements' for context '{context}' is not a list. Skipping check.")
        ...
        if isinstance(tone_list, list) and tone_list: # Check if list and not empty
            ...
            found_tone = any(
                ...
                for t in tone_list if isinstance(t, str) and t # Check t is string and not empty
            )
            if not found_tone:
                ...
                suggestions.append(f"Adjust tone to be more '{tone_list[0]}' for {context}")
        elif "tone" in situation and not isinstance(tone_list, list):
             self.logger.warning(f"'tone' for context '{context}' is not a list. Skipping check.")
    ...
    def _apply_care_guidelines(self, text_lower: str) -> Dict[str, Any]:
        ...
        if not isinstance(self.care_guidelines, dict):
             self.logger.warning(f"Care guidelines structure is invalid (not a dict). Skipping application.")
        ...
        for guideline_id, rules in self.care_guidelines.items():
             ...
             if not isinstance(rules, dict):
                  self.logger.warning(f"Rule structure for guideline '{guideline_id}' is invalid (not a dict). Skipping.")
                  continue
             ...
             if isinstance(required, list) and required: # Check if list and not empty
                 ...
                 found_required = any(
                     re.search(rf"\b{re.escape(req.lower())}\b", text_lower)
                     for req in required if isinstance(req, str) and req
                 )
                 if not found_required:
                  ...
                  self.logger.warning(f"'required' for guideline '{guideline_id}' is not a list. Skipping check.")
             ...
             if isinstance(avoid, list) and avoid: # Check if list and not empty
                 ...
                 found_avoid = any(
                     ...
                     for a in avoid if isinstance(a, str) and a
                 )
                 if found_avoid:
                     score += self._CARE_AVOID_PRESENT_PENALTY # Use constant
                     suggestions.append(
                         rules.get("warning", f"Revise approach regarding {guideline_id}")
                     )
             elif "avoid" in rules and not isinstance(avoid, list):
                  self.logger.warning(f"'avoid' for guideline '{guideline_id}' is not a list. Skipping check.")


        return {"score": max(0.0, score), "suggestions": suggestions}

    def _extract_phrase_context(self, text_lower: str, phrase_lower: str, window: int = 50) -> str:
        ...
        try:
            ...
            if not match:
                self.logger.debug(f"Phrase '{phrase_lower}' not found for context extraction.")
            ...
            return f"{prefix}{text_lower[start:end]}{suffix}"
             ...
             self.logger.error(f"Error extracting context for phrase '{phrase_lower}': {e}", exc_info=True)
    ...
    def get_topic_guidelines(self, topic_id: str) -> Optional[Dict[str, Any]]:
        ...
        self.logger.warning("Sensitivity topics not loaded correctly, cannot get guidelines.")
        return None
    ...
    def list_sensitive_topics(self) -> List[str]:
        ...
        self.logger.warning("Sensitivity topics not loaded correctly, cannot list topics.")
        return []
    ...
    def get_situation_guidelines(self, situation: str) -> Optional[Dict[str, Any]] :
        ...
        self.logger.warning("Life situations not loaded correctly, cannot get guidelines.")
        return None
    ...
    def suggest_pastoral_response(self, topic_id: str) -> Optional[Dict[str, Any]]:
        ...
        if not isinstance(self.sensitivity_topics, dict):
             self.logger.warning("Sensitivity topics not loaded correctly, cannot suggest response.")
             return None
        ...
        if isinstance(topic, dict):
            return {
             ...
             self.logger.warning(f"Rule structure for topic '{topic_id}' is invalid (not a dict). Cannot suggest response.")
             return None
        else:
