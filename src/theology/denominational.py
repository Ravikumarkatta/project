...
class DenominationalAdjuster:
        ...
        try:
            ...
            if not rules_file.is_file():
                ...
                if rules_path == "config/theological_rules.json":
                     self.logger.error(f"Default rules file not found or not a file: {rules_path}. Denominational checks may be limited.")
                     return {}
                else:
            ...
            self.logger.exception(f"Invalid JSON in {rules_path}. Cannot load denominational rules.")
    ...
    def _check_phrase_list_or_string(self, text_lower: str, phrases: Union[str, List[str]]) -> bool:
        ...
        if isinstance(phrases, str) and phrases:
            ...
            # Handle list case
            return any(
                isinstance(p, str) and p and re.search(rf"\b{re.escape(p.lower())}\b", text_lower)
                for p in phrases
            )
        ...
        if isinstance(self.variations, dict):
            for topic, rules in self.variations.items():
                ...
                if not isinstance(rules, dict):
                     self.logger.warning(f"Invalid structure for variation rule '{topic}'. Expected dict.")
                     continue
                ...
                if isinstance(default_term, str) and default_term and \
                   isinstance(denom_term, str) and denom_term and \
                   denom_term.lower() != default_term.lower():
                    ...
                    pattern = re.compile(rf"\b{re.escape(default_term)}\b", re.IGNORECASE)
                    if pattern.search(adjusted_text): # Search in original case text
                        adjusted_text = pattern.sub(denom_term, adjusted_text) # Replace using denom_term (original case)
                        issues.append(
             ...
             self.logger.warning("Denominational variations rules not loaded correctly (not a dict). Skipping terminology adjustment.")
    ...
    def _extract_term_context(self, text_lower: str, term_lower: str, window: int = 50) -> str:
        ...
        try:
            ...
            if not match:
                self.logger.debug(f"Term '{term_lower}' not found for context extraction.")
            ...
            return f"{prefix}{text_lower[start:end]}{suffix}"
             ...
             self.logger.error(f"Error extracting context for term '{term_lower}': {e}", exc_info=True)
    ...
    def get_sensitivity_terms(self, denomination: str) -> Dict[str, List[str]]:
        ...
        self.logger.warning("Denominational sensitivities not loaded correctly, cannot get terms.")
        return {}
    ...
    def _check_mixed_terminology(
        ...
        if not isinstance(self.variations, dict):
             self.logger.warning("Denominational variations rules not loaded correctly (not a dict). Skipping mixed terminology check.")
             return issues

        for topic, rules in self.variations.items():
             # Ensure rules is a dict
             if not isinstance(rules, dict):
                  self.logger.warning(f"Invalid structure for variation rule '{topic}'. Skipping mixed terminology check for this topic.")
                  continue

             variations = rules.get("variations", {})
             # Ensure variations is a dict
             if not isinstance(variations, dict):
                  self.logger.warning(f"Invalid 'variations' structure within rule '{topic}'. Skipping.")
                  continue
