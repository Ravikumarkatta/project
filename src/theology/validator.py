                    ...
                    with open(rule_file) as f:
                        ...
                        if "heterodox_statements" in data:
                            if "general" not in rules["heretical"]:
                                rules["heretical"]["general"] = {"patterns": []}
                            ...
                            for stmt in data.get("heterodox_statements", []):
                                ...
                                rules["heretical"]["general"]["patterns"].append(
                                    pattern
                                )
        ...
        try:
            ...
            for category, content in self.rules["doctrinal"].items():
                ...
                for key_statement in content.get("key_statements", []):
                    if (
                        key_statement.lower() in statement.lower()
                        or statement.lower() in key_statement.lower()
                    ):
                ...
                for keyword in content.get("keywords", []):
                    if re.search(
                        r"\b" + re.escape(keyword) + r"\b", statement, re.IGNORECASE
                    ):
            ...
            if not doctrinal_match and not heretical_match:
                ...
                if all_doctrinal_statements:
                    try:
                        ...
                        doctrinal_embeddings = self.model.encode(
                            all_doctrinal_statements
                        )
                        doctrinal_sims = cosine_similarity(
                            [statement_embedding], doctrinal_embeddings
                        )[0]
