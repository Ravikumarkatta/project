...
class VerseReference:
    ...
    def __init__(self, book: str, chapter: int, verse: int, end_verse: Optional[int] = None):
        self.book = book
...
class VerseResolver:
    ...
    def __init__(self, bible_data_path: Optional[str] = None):
        ...
        self.compiled_patterns = [re.compile(pattern) for pattern in self.verse_patterns]
    ...
    def _load_bible_data(self, bible_data_path: Optional[str] = None) -> Dict:
        ...
        if not bible_data_path:
            bible_data_path = Path(__file__).parent.parent.parent / "data" / "processed" / "kjv_processed.json"
