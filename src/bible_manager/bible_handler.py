...
class BibleHandler:
    def __init__(
        self, download_dir: str = "downloads", data_dir: str = "data/raw/bibles"
    ):
    ...
    async def download_bible_version(self, version: str) -> str:
        ...
        if version not in sources:
            raise HTTPException(
                status_code=400, detail=f"Version {version} not supported"
            )
        ...
        try:
            ...
            raise HTTPException(
                status_code=500, detail=f"Failed to download {version} Bible"
            )
    ...
    async def process_uploaded_bible(self, file_content: str, version: str) -> Dict:
        ...
        try:
            ...
            raise HTTPException(
                status_code=500, detail="Failed to process uploaded Bible"
            )
