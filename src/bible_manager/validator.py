...
class BibleDataValidator:
        ...
        if config_path:
            ...
            if config_file.exists():
                try:
                    ...
                    logging.getLogger(__name__).error(f"Failed to load config from {config_path}: {e}. Using defaults.")
            else:
    ...
    def check_raw_data(self) -> bool:
        ...
        try:
            ...
            if not bible_files:
                self.logger.warning("No primary bible data files (.json, .xml) found in raw directory. Checking for .txt")
                # Fallback check for .txt if needed, depending on expected raw formats
                bible_files = list(self.raw_dir.glob("*.txt"))
                if not bible_files:
                    self.logger.error("No suitable bible data files found in raw directory.")
                    return False # Correctly placed return
    ...
    def check_processed_data(self) -> bool:
        ...
        try:
            ...
            processed_file_json = self.processed_dir / "bible.json" # Example primary format
            ...
            if processed_file_json.exists():
                processed_file_to_check = processed_file_json
                 ...
                 self.logger.info(f"Found processed data as DB: {processed_file_db}. Skipping JSON validation logic.")
                 # Add DB-specific validation here if needed
                 return True # Assuming DB presence is sufficient for this check
            else:
                self.logger.error(f"No processed bible data found at expected locations ({processed_file_json}, {processed_file_db})")
                return False # Correctly placed return
            ...
            if processed_file_to_check and processed_file_to_check.suffix == ".json":
                ...
                if self.config.required_books is None:
                     self.logger.error("Validation config error: required_books is None.")
                     return False
                ...
                if missing_books:
                    self.logger.error(f"Missing required books in processed data: {missing_books}")
                    return False # Correctly placed return
                ...
                for book in self.config.required_books:
                    if book not in data:
                        ...
                        self.logger.error(f"Required book '{book}' unexpectedly not found in data after initial check.")
                        return False
                    # Calculate verse count for the book
                    verse_count = 0
                    if isinstance(data[book], dict):
                        verse_count = sum(len(verses) for verses in data[book].values() if isinstance(verses, dict))
                    else:
                        self.logger.warning(f"Unexpected data structure for book '{book}'. Expected dict of chapters.")
            ...
            self.logger.error(f"Processed data verification failed unexpectedly: {str(e)}")
            return False # Correctly placed return
    ...
    def verify_verse_references(self) -> bool:
        ...
        try:
            ...
            if not verse_file.exists():
                self.logger.info(f"Verse reference file not found at {verse_file}. Skipping verification.")
            ...
            if not isinstance(refs, list):
                 self.logger.error(f"Verse reference file {verse_file} does not contain a list.")
                 return False
            ...
            for i, ref in enumerate(refs):
                if not isinstance(ref, dict):
                     self.logger.error(f"Invalid item type in verse references (index {i}): expected dict, got {type(ref)}")
                     valid = False
                     continue # Skip to next item

                # Check for required fields
                missing_fields = [field for field in self.config.required_fields if field not in ref]
                if missing_fields:
                    self.logger.error(f"Invalid verse reference format (index {i}): missing fields {missing_fields} in {ref}")
                    valid = False
                    continue
                ...
                try:
                    ...
                    self.logger.error(f"Invalid chapter/verse numbers (index {i}): non-integer values in {ref}")
                    valid = False
            ...
            self.logger.error(f"Verse reference verification failed unexpectedly: {str(e)}")
            return False # Correctly placed return
