...
class BibleConverter:
    ...
    def convert(self, input_path: str, input_format: Optional[str] = None) -> Dict[str, Any]:
    ...
    def _detect_format(self, file_path: str) -> Optional[str]:
        ...
        try:
            with open(file_path, 'r', encoding='utf-8') as f:
                ...
                if '*** START OF THE PROJECT GUTENBERG' in content: return 'gutenberg'
                return 'txt'
        except Exception as e:
    ...
    def _validate_bible_data(self, bible_data: Dict[str, Any]) -> bool:
        ...
        for book in bible_data["books"]:
            if "code" not in book or book["code"] not in self.book_name_to_code.values():
                logger.error(f"Invalid or missing book code: {book.get('code', 'missing')}")
                return False
            ...
            for chapter in book["chapters"]:
                if "verses" not in chapter or not chapter["verses"]:
                    logger.error(f"No verses in chapter {chapter.get('number', 'unknown')} of book {book.get('name', 'unknown')}")
                    return False
            ...
            for book_name, chapters in raw_data.items():
                ...
                for chapter_num, verses in chapters.items():
                    ...
                    for verse_num, verse_text in verses.items():
                        chapter['verses'].append({'number': verse_num, 'text': verse_text})
        ...
        with open(file_path, 'r', encoding='utf-8') as f:
            for line in f:
                ...
                if line.startswith('\\id '):
                    ...
                    book_name = next((name for name, code in self.book_name_to_code.items() if code == book_code), None)
                    if book_name:
                        current_book = {'name': book_name, 'code': book_code, 'chapters': []}
            ...
            with open(file_path, 'r', encoding='utf-8') as f:
                ...
                for row in reader:
                    ...
                    if book_name not in book_dict:
                        book_dict[book_name] = {'name': book_name, 'code': book_code, 'chapters': {}}
    ...
    def _read_xml(self, file_path: str) -> Dict[str, Any]:
        try:
            ...
            root = tree.getroot()
            bible_data = {'metadata': {'title': root.get('title', os.path.basename(file_path))}, 'books': []}
        ...
        with open(file_path, 'r', encoding='utf-8') as f:
            for line in f:
                ...
                try:
                    ...
                    self.logger.warning(f"Skipping malformed TXT line in {file_path}: {line}")
    ...
    def _read_gutenberg(self, file_path: str) -> Dict[str, Any]:
        ...
        pattern_old = re.compile(r"the\s+(first|second|third|fourth|fifth)?\s*book\s+of\s+[\w\s]+?:\s*called\s+(.+)", re.IGNORECASE)
        pattern_book = re.compile(r"the\s+book\s+of\s+(.+)", re.IGNORECASE)
        pattern_gospel = re.compile(r"the\s+gospel\s+according\s+to\s+(.+)", re.IGNORECASE)
            ...
            for line in lines:
                ...
                if "called" in lower_line:
                    ...
                    if match:
                        ...
                        if any(kw in candidate.lower() for kw in ["revelation", "psalms", "acts"]):
                            new_book = next((name for name in self.book_name_to_code if name.lower() in candidate.lower()), None)
                ...
                if new_book:
                    ...
                    for num, word in [("1", "first"), ("2", "second"), ("3", "third"), ("4", "fourth"), ("5", "fifth")]:
                        new_book = new_book.replace(word, num)
                    new_book = new_book.replace("saint ", "").replace("the prophet ", "").strip()
                    ...
                    if book_code == 'UNKNOWN':
                        self.logger.warning(f"Unknown book name in Gutenberg: {current_book_name}")
                        current_book_name = None
                        continue
                    if current_book_name not in book_dict:
                        book_dict[current_book_name] = {'name': current_book_name, 'code': book_code, 'chapters': {}}
                    ...
                    if line and line[0].isdigit() and ':' in line[:5]:
                        try:
                            ...
                            if chapter_num not in book_dict[current_book_name]['chapters']:
                                book_dict[current_book_name]['chapters'][chapter_num] = {'number': chapter_num, 'verses': []}
                            book_dict[current_book_name]['chapters'][chapter_num]['verses'].append({
                                'number': str(verse_num),
                                'text': text.strip()
                            })
                        except Exception:
                            self.logger.warning(f"Skipping malformed Gutenberg line in {file_path}: {line}")
