...
class Trainer:
    ...
    def setup_data(self) -> None:
        ...
        try:
            ...
            self.logger.info(
                f"Loaded {len(self.train_data)} training samples and {len(self.val_data)} validation samples"
            )
        except Exception as e:
    ...
    def setup_training_components(self) -> None:
        ...
        try:
            ...
            self.logger.info(
                f"Initialized optimizer with learning rate {lr} and {warmup_steps} warmup steps"
            )
        ...
        for epoch in range(max_epochs):
            ...
            for batch_idx, batch in enumerate(self.train_loader):
                ...
                if use_mixed_precision and scaler is not None:
                    ...
                    loss = (
                        self.criterion(outputs["logits"], labels) / accumulation_steps
                    )
                    loss.backward()
                    if (batch_idx + 1) % accumulation_steps == 0:
                        torch.nn.utils.clip_grad_norm_(
                ...
                if batch_idx % log_every_n_steps == 0:
                    self.logger.info(
                        f"Epoch {epoch+1}/{max_epochs}, Batch {batch_idx}, Loss: {loss.item() * accumulation_steps:.4f}"
                    )
                ...
                self.metrics_collector.track_validation_score(
                    {"validation_score": validation_score}
                )
                self.metrics_collector.track_verse_accuracy(
                    outputs["verse_logits"], verse_positions
                )

                save_every_n_steps = self.config["training"]["checkpoint"][
                    "save_every_n_steps"
                ]
                if batch_idx > 0 and batch_idx % save_every_n_steps == 0:
                    checkpoint_dir = self.config.get("model", {}).get(
                        "checkpoint_dir", "models/checkpoints"
                    )
                    os.makedirs(checkpoint_dir, exist_ok=True)
                    checkpoint_path = os.path.join(
                        checkpoint_dir, f"checkpoint_epoch{epoch+1}_step{batch_idx}.pt"
                    )
                    torch.save(
                        {
                            "epoch": epoch,
                    ...
                    self.logger.info(
                        f"Saved checkpoint at epoch {epoch+1}, step {batch_idx}"
                    )

            avg_train_loss = epoch_loss / len(self.train_loader)
            val_loss = self.validate()
            self.logger.info(
                f"Epoch {epoch+1}/{max_epochs}: Train Loss = {avg_train_loss:.4f}, Val Loss = {val_loss:.4f}"
            )

            save_best_only = self.config["training"]["checkpoint"]["save_best_only"]
            if save_best_only and val_loss < best_val_loss:
                best_val_loss = val_loss
                model_save_path = self.config.get("model", {}).get(
