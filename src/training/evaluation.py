...
class BibleEvaluator:
    ...
    def compute_metrics(
        ...
        if verse_logits is not None and verse_labels is not None:
            ...
            verse_accuracy = (
                (verse_preds[verse_mask] == verse_labels[verse_mask])
                .float()
                .mean()
                .item()
            )
    ...
    def evaluate(self, epoch: int = 0) -> Dict[str, float]:
        ...
        # Average metrics
        metrics = {
            metric: value / num_batches for metric, value in total_metrics.items()
        }
...
if __name__ == "__main__":
    ...
    model = BiblicalTransformer(
        BiblicalTransformerConfig(**config.get("model_params", {}))
    )
    val_data = BibleDataset("val", config.get("data", {}))  # Placeholder
    val_loader = DataLoader(
        val_data, batch_size=config.get("training", {}).get("batch_size", 16)
    )
