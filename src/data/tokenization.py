...
class BibleTokenizer:
    ...
    def tokenize(self, text: str, return_tensors: Optional[str] = None) -> Dict[str, Any]:
        """
        ...
        """
        if not isinstance(text, str):
            logger.error(f"Input to tokenize must be a string, but received {type(text)}")
            raise TypeError("Input text must be a string.")
        ...
        try:
            ...
            tensor_type = return_tensors if return_tensors is not None else self.config.output_type
            if tensor_type not in ["pt", "np", "jax", None]:
                 logger.warning(f"Invalid return_tensors type '{tensor_type}'. Using default '{self.config.output_type}'.")
            ...
            logger.critical(f"Tokenization failed after normalization: {e}", exc_info=True)
            raise RuntimeError("Fatal tokenization error.") from e

    def load_state_dict(self, checkpoint_path: str, **kwargs) -> Dict[str, torch.Tensor]:
        """
        ...
        safetensors_device = kwargs.get("device", self.config.device) # safetensors uses 'device' arg
        ...
        try:
            ...
            with warnings.catch_warnings():
                ...
                state_dict = torch.load(
                    ...
                    weights_only=kwargs.get("weights_only", True), # Prefer weights_only=True for security
                    pickle_module=kwargs.get("pickle_module", pickle),
                    **{k: v for k, v in kwargs.items() if k not in ['map_location', 'weights_only', 'pickle_module', 'device']} # Pass other kwargs
                )
            logger.info("Standard torch.load successful.")
            return state_dict
        except Exception as e:
            original_error = e
            logger.warning(f"Standard torch.load failed: {e}. Attempting recovery methods.")

        # --- Recovery Attempts ---

        # Define recovery methods and their specific arguments/messages
        recovery_attempts = [
            (self._try_safetensors_load, {'path': checkpoint_path, 'device': safetensors_device, **kwargs}, "Safetensors load failed: Format mismatch or version issue."),
            (self._try_mmap_load, {'path': checkpoint_path, 'map_location': map_location, **kwargs}, "Memory-mapped loading failed: File might be corrupted or incompatible."),
            (self._try_legacy_load, {'path': checkpoint_path, 'map_location': map_location, **kwargs}, "Legacy pickle load failed: Unsupported pickle protocol or data corruption."),
        ]
        ...
        for i, (func, func_kwargs, error_msg) in enumerate(recovery_attempts):
            try:
                ...
                if result is not None:
                    logger.warning(f"Recovery step {i+1} ({func.__name__}) succeeded with potential warning: {error_msg}")
                    return result
                logger.debug(f"Recovery step {i+1} ({func.__name__}) returned None.")
            except Exception as recovery_e:
                logger.debug(f"Recovery step {i+1} ({func.__name__}) failed with error: {recovery_e}")

        # If all recovery attempts fail
        logger.critical(f"All recovery attempts failed for checkpoint: {checkpoint_path}")
        raise RuntimeError(
    ...
    def _try_mmap_load(self, path: str, **kwargs) -> Optional[Dict[str, torch.Tensor]]:
        ...
        try:
            ...
            if sys.platform == "win32" and sys.version_info < (3, 8):
                 logger.warning("Memory-mapped loading on Windows is less reliable in Python versions < 3.8.")
            ...
            with open(path, "rb") as f:
                ...
                with warnings.catch_warnings():
                    ...
                    return torch.load(
                         ...
                         **{k: v for k, v in kwargs.items() if k != 'map_location' and k != 'device'} # Pass other kwargs, exclude device
                    )
    ...
    def _try_safetensors_load(self, path: str, **kwargs) -> Optional[Dict[str, torch.Tensor]]:
        ...
        try:
            ...
            return safe.load_file(path, device=device, **{k: v for k, v in kwargs.items() if k != 'device'}) # Pass other kwargs, exclude device
        except Exception as e:
            # safetensors might raise different errors (e.g., safetensors_rust.SafetensorError)
            logger.debug(f"Safetensors load failed: {e}")
            return None

    def _try_legacy_load(self, path: str, **kwargs) -> Optional[Dict[str, torch.Tensor]]:
...
if __name__ == "__main__":
    ...
    try:
        ...
        tokenized_output = tokenizer.tokenize(text, return_tensors="pt") # Request PyTorch tensors
        print(f"\nTokenized Output (input_ids shape): {tokenized_output['input_ids'].shape}")
        print(f"Tokenized Output (attention_mask shape): {tokenized_output['attention_mask'].shape}")
        ...
        decoded_text = tokenizer.base_tokenizer.decode(tokenized_output['input_ids'][0], skip_special_tokens=True)
        ...
        dummy_state_dict = {"encoder.layer.0.weight": torch.randn(768, 768), "decoder.bias": torch.randn(10)}
        ...
        try:
            loaded_state_dict_pt = tokenizer.load_state_dict(dummy_checkpoint_path, map_location='cpu')
        ...
        try:
            ...
            loaded_state_dict_sf = tokenizer.load_state_dict(dummy_safetensors_path, device='cpu')
