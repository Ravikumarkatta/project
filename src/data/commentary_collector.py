...
class CommentaryCollector:
    ...
    def load_config(self):
        ...
        try:
            ...
            sources_path = os.path.join(os.path.dirname(self.config_path), "commentary_sources.json")
            with open(sources_path, 'r') as f:
                sources_config = json.load(f)
                self.sources = [CommentarySource(**source) for source in sources_config["sources"]]
        ...
        async with aiohttp.ClientSession() as session:
            async with session.get(source.url) as response:
                if response.status == 200:
                    ...
                    logger.error(f"Web scraping failed for {source.name}: {response.status}")
                    return []
    
    def collect_from_local(self, source: CommentarySource) -> List[Dict[str, Any]]:
        """Collect commentary data from local files."""
        try:
            file_path = os.path.join(self.raw_dir, source.url)  # url field contains local path
    ...
    def _process_api_response(self, data: Dict[str, Any], source: CommentarySource) -> List[Dict[str, Any]]:
    ...
    def _process_web_content(self, html: str, source: CommentarySource) -> List[Dict[str, Any]]:
    ...
    def _process_xml_content(self, content: str, source: CommentarySource) -> List[Dict[str, Any]]:
        """Process XML content."""
        entries = []
        parser_config = source.parser_config or {}
    ...
    def _process_text_content(self, content: str, source: CommentarySource) -> List[Dict[str, Any]]:
