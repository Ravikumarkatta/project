...
class BibleAIDataPipeline:
    ...
    def run_pipeline(
        self, augment: bool = True, max_augmentations: int = 3
        ...
        for bible_file in os.listdir(bibles_dir):
            if bible_file.endswith((".xml", ".json", ".txt")):
                ...
                bible_data = self.preprocessor.process_bible_file(
        ...
        if augment:
            ...
            augmented_qa = self.augmenter.augment_qa_batch(
                qa_pairs, refs, intensity=0.2
            )
            ...
            for (aug_q, aug_a), orig in zip(augmented_qa, instruction_data):
                ...
                augmented_instruction_data.append(
                    {"instruction": instruction, "input": input_text, "output": aug_a}
                )
    ...
    for item in instruction_data:
        ...
        verse_positions = (
            verse_output["verse_logits"].argmax(dim=-1) > 0
        ).float()  # Binary mask for verse positions

        # Add to dataset
        item["input_ids"] = input_ids
        item["attention_mask"] = attention_mask
        item["verse_positions"] = verse_positions
        item["labels"] = self.tokenizer.tokenize(
            item["output"],
            return_tensors="pt",
            max_length=512,
            truncation=True,
        ...
        train_path = os.path.join(
            self.preprocessor.processed_dir, "train_instruction.json"
        )
...
if __name__ == "__main__":
    ...
    print(
        f"Created DataLoaders with {len(train_loader)} training batches and {len(val_loader)} validation batches."
    )
