...
class BiblicalDataAugmenter:
    ...
    def _load_synonyms(self) -> Dict[str, List[str]]:
        ...
        if not os.path.exists(synonyms_path):
            logger.warning(f"Synonyms file not found at {synonyms_path}. Synonym replacement will be limited.")
            return {}
    ...
    def _load_paraphrases(self) -> Dict[str, List[str]]:
        ...
        if not os.path.exists(paraphrases_path):
            logger.warning(f"Paraphrases file not found at {paraphrases_path}. Paraphrase augmentation will be limited.")
            return {}
        
        try:
            with open(paraphrases_path, 'r') as f:
                paraphrases = json.load(f)
            logger.info(f"Loaded {len(paraphrases)} paraphrase sets from {paraphrases_path}")
            return paraphrases
        except Exception as e:
    ...
    def augment_dataset(self, texts: List[str], techniques: List[str] = None) -> List[str]:
        """
        ...
        logger.info(f"Augmented dataset from {len(texts)} to {len(augmented_dataset)} samples")
    ...
    def _synonym_replacement(self, text: str) -> str:
        ...
        """
        if not self.synonyms or random.random() > self.config.get("synonym_replacement_prob", 0.3):
            return text
        ...
        replace_indices = random.sample(range(len(words)), min(n_to_replace, len(words)))
    ...
    def _paraphrase(self, text: str) -> str:
        ...
        """
        if not self.paraphrases or random.random() > self.config.get("paraphrase_prob", 0.2):
            return text
    ...
    def _theological_concept_insertion(self, text: str) -> str:
        ...
        """
        if random.random() > self.config.get("theological_concept_insertion_prob", 0.15):
            return text
        ...
        if random.random() < 0.5:  # Beginning
            ...
            sentences[insert_idx] = re.sub(r'([.!?])$', f", {concept}\\1", sentences[insert_idx])
