...
class EmbeddingPipeline:
    ...
    def setup_storage(self):
        ...
        self.commentary_index_path = os.path.join(self.embeddings_dir, "commentary_embeddings.h5")
    ...
    def generate_embeddings(self, texts: List[str], show_progress: bool = True) -> np.ndarray:
        ...
        # Process in batches
        for i in tqdm(range(0, len(texts), self.embedding_config.batch_size),
    ...
    def save_verse_embeddings(self, embeddings: np.ndarray, references: List[str]):
        ...
        try:
            ...
            logger.info(f"Saved {len(embeddings)} verse embeddings to {self.verse_index_path}")
        except Exception as e:
    ...
    def save_commentary_embeddings(self, embeddings: np.ndarray, metadata: List[Dict]):
        ...
        try:
            ...
            logger.info(f"Saved {len(embeddings)} commentary embeddings to {self.commentary_index_path}")
        except Exception as e:
            ...
            for entry in entries:
                if "content" in entry:
                    ...
                    metadata.append({
                        ...
                        **{k: v for k, v in entry.items() if k not in ["content", "source", "reference", "collected_at"]}
                    })
...
def main():
    ...
    for file in bible_files:
        if file.endswith(('.xml', '.json', '.txt')):
            ...
            bible_data[translation] = preprocessor.process_bible_file(file_path, translation)
