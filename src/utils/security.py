...
BIBLE_REFERENCE_PATTERN = re.compile(
    r"^[\w\s\d\:\-\.]+$"
)  # Allows Bible references like "John 3:16-17"
...
class SecurityManager:
    ...
    def __init__(self, config_path: Optional[str] = None):
        ...
        if not self.secret_key:
            ...
            print(
                "Warning: Using generated secret key. Set BIBLE_AI_SECRET_KEY environment variable for production."
            )
    ...
    def sanitize_input(
        self, text: str, pattern: Optional[re.Pattern] = None
    ) -> Tuple[str, bool]:
        """
    ...
    def verify_password(
        self, stored_hash: str, stored_salt: str, provided_password: str
    ) -> bool:
        """
    ...
    def generate_token(self, user_id: str, expiry_seconds: Optional[int] = None) -> str:
        ...
        # Create signature
        signature = hmac.new(
            self.secret_key.encode("utf-8"), payload_b64.encode("utf-8"), hashlib.sha256
        ).digest()
    ...
    def track_login_attempt(self, identifier: str, success: bool) -> Tuple[bool, int]:
        ...
        tracking_info = self.failed_logins.get(
            identifier, {"count": 0, "last_attempt": 0}
        )

        # Check lockout expiration
        lockout_expiry = tracking_info["last_attempt"] + self.config["lockout_time"]
        if (
            tracking_info["count"] >= self.config["max_login_attempts"]
            and current_time < lockout_expiry
        ):
            # Still locked out
        ...
        attempts_remaining = max(
            0, self.config["max_login_attempts"] - tracking_info["count"]
        )
    ...
    def validate_api_request(
        self, endpoint: str, method: str, user_role: str, required_role: str
    ) -> bool:
        """
        ...
        role_levels = {"admin": 4, "editor": 3, "user": 2, "guest": 1, "": 0}
