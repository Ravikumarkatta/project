...
class VerseEmbedder:
    def __init__(
        ...
        if self.data_format == "nested_dict" and self.bible_data:
            ...
            sample_data = {
                sample_book: {
                    k: v for k, v in list(self.bible_data[sample_book].items())[:1]
                }
            }
            logger.info(
                f"Sample of retrieved data: {json.dumps(sample_data, indent=2)}"
            )
        elif self.data_format in [
            "books_list_with_number_dict",
            "books_list_with_number_list",
        ] and self.bible_data.get("books"):
            logger.info(
                f"Sample of retrieved data: {json.dumps(list(self.bible_data['books'])[:1], indent=2)}"
            )
        else:
    ...
    def _detect_format(self) -> str:
        ...
        if isinstance(self.bible_data, dict):
            if "books" in self.bible_data and isinstance(
                self.bible_data["books"], list
            ):
    ...
    def generate_embeddings(self, version: str) -> Dict[str, list]:
        ...
        if self.data_format == "nested_dict":
            ...
            for book_entry in self.bible_data["books"]:
                ...
                for chapter_entry in chapters:
                    ...
                    if not chapter_num:
                        logger.warning(
                            f"Chapter entry in {book_name} missing 'number', skipping"
                        )
                        continue
                    ...
                    if not isinstance(verses, dict):
                        logger.warning(
                            f"Verses for {book_name} {chapter_num} is not a dict, skipping"
                        )
                        continue
            ...
            for book_entry in self.bible_data["books"]:
                ...
                for chapter_entry in chapters:
                    ...
                    if not isinstance(verses, list):
                        logger.warning(
                            f"Verses for {book_name} {chapter_num} is not a list, skipping"
                        )
                        continue

                    for verse_entry in verses:
                        if (
                            not isinstance(verse_entry, dict)
                            or "number" not in verse_entry
                            or "text" not in verse_entry
                        ):
                            logger.warning(
                                f"Invalid verse entry in {book_name} {chapter_num}: {verse_entry}"
                            )
                            continue
        ...
        embeddings = self.model.encode(
            verse_texts, convert_to_tensor=False, show_progress_bar=True, batch_size=32
        )

        embeddings_dict = {
            key: embedding.tolist() for key, embedding in zip(verse_keys, embeddings)
        }
    ...
    def load_embeddings(self, version: str) -> Dict[str, list]:
        ...
        try:
            ...
            logger.warning(
                f"No embeddings found for version '{version}' at {embeddings_path}"
            )
            return {}
        except json.JSONDecodeError as e:
            logger.error(
                f"Failed to decode embeddings file {embeddings_path}: {str(e)}"
            )
            return {}
...
if __name__ == "__main__":
    ...
    if loaded_embeddings:
        print(
            "Sample embedding for Genesis_50_1:",
            loaded_embeddings.get("Genesis_50_1")[:5],
        )
    else:
