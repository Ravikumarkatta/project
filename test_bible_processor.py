...
class TestBibleProcessor(unittest.TestCase):
    ...
    def test_normalize_book_name(self):
        ...
        self.assertEqual(
            self.processor.normalize_book_name("The Revelation"), "Revelation"
        )
    ...
    def test_identify_verse_reference(self):
        ...
        # Test non-verse line
        self.assertIsNone(
            self.processor.identify_verse_reference("This is not a verse")
        )
    ...
    def test_process_bible_text(self):
        ...
        try:
            report = self.processor.process_bible_text(
                input_path, output_path, "json", True
            )
            ...
            self.assertEqual(
                output_data["Genesis"][1][1],
                "In the beginning God created the heaven and the earth.",
            )
            self.assertEqual(
                output_data["Genesis"][1][2],
    ...
    def test_validation(self):
        ...
        self.assertTrue(
            any("Low verse count" in warning for warning in report["warnings"])
        )

        # It should warn about missing books
        self.assertTrue(
            any("Missing books" in warning for warning in report["warnings"])
        )
